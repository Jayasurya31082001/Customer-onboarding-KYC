...
public class GlobalExceptionHandler {
    ...
    public ProblemDetail handleInvalidDocument(InvalidDocumentException exception, HttpServletRequest request) {
        return createProblemDetail(
                HttpStatus.BAD_REQUEST,
                "Invalid document",
                exception.getMessage(),
                request,
                "https://example.com/problems/invalid-document"
        );
    }

    @ExceptionHandler(DocumentStorageException.class)
    public ProblemDetail handleDocumentStorage(DocumentStorageException exception, HttpServletRequest request) {
        LOGGER.error("Document processing failed. correlationId={}, message={}", MDC.get("correlationId"), exception.getMessage(), exception);
        return createProblemDetail(
                HttpStatus.INTERNAL_SERVER_ERROR,
                "Document processing failed",
                exception.getMessage(),
                request,
                "https://example.com/problems/document-processing-failed"
        );
    }

    @ExceptionHandler(Exception.class)
    public ProblemDetail handleUnexpected(Exception exception, HttpServletRequest request) {
        LOGGER.error("Unexpected error. correlationId={}", MDC.get("correlationId"), exception);
        return createProblemDetail(
                HttpStatus.INTERNAL_SERVER_ERROR,
