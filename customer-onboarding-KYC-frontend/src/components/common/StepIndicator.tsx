import { ONBOARDING_STEP_ENTRIES, OnboardingStep } from "../../types/onboarding.types";
...
export const StepIndicator = ({ currentStep }: StepIndicatorProps) => {
  const currentIndex = ONBOARDING_STEP_ENTRIES.findIndex((entry) => entry.step === currentStep);

  return (
    <ol className="grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-5" aria-label="Onboarding progress">
      {ONBOARDING_STEP_ENTRIES.map((entry, index) => {
